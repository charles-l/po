...
(run-test `(string-ref (string-set!
			 (make-string 1)
			 0 #\B) 0)	'eq? #\B)
(run-test '(labels ((a (lambda () 123)))
	     (a))			'eq? 123)
(run-test '(labels ((a (lambda (x) x)))
	     (a 4))			'eq? 4)
(run-test '(labels ((a (lambda (x y z) x)))
	     (a 4 1 2)) 		'eq? 4)
(run-test '(labels ((a (lambda (x y z) y)))
	     (a 4 1 2)) 		'eq? 1)
(run-test '(labels ((a (lambda (x y z) z)))
	     (a 4 1 2)) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) x)))
	       (a 5 2 7))) 		'eq? 5)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) y)))
	       (a 5 2 7))) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) z)))
	       (a 5 2 7))) 		'eq? 7)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) (+ x x))))
	      (a 5 2 7))) 		'eq? 10)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) (+ x z))))
	      (a 5 2 7))) 		'eq? 12)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z) (+ x (- z 3)))))
	      (a 5 2 7))) 		'eq? 9)
(run-test '(let ((n 0))
	     (labels ((a (lambda (x y z)
			   (let ((k (+ x 1)))
			     (+ k (- z 3))))))
	      (a 5 2 7))) 		'eq? 10)
(run-test '(labels ((a (lambda () 123)) (b (lambda (x) (- 3 x))))
	     (b (a)))			'eq? -120)
