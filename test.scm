...
(run-test `(string-set!
	     ...
	     0
	     #\B)		'equal? "B")
(run-test `(string-ref (string-set!
			 (make-string 5)
			 4 #\A)
		       4)	'eq? #\A)
(run-test `(string-ref (string-set!
			 ...
			 0 #\B)
		       0)	'eq? #\B)
(run-test '(labels ((a (code () () 123)))
	     (procedure? (closure a)))		'eq? #t)
(run-test '(labels ((a (code () () 123)))
	     (funcall (closure a)))	'eq? 123)
(run-test '(labels ((a (code (x) () x)))
	     (funcall (closure a) 4))	'eq? 4)
(run-test '(labels ((a (code (x y z) () x)))
	     (funcall (closure a) 4 1 2))	'eq? 4)
(run-test '(labels ((a (code (x y z) ()  y)))
	     (funcall (closure a) 4 1 2)) 		'eq? 1)
(run-test '(labels ((a (code (x y z) ()  z)))
	     (funcall (closure a) 4 1 2)) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () x)))
	       (funcall (closure a) 5 2 7))) 		'eq? 5)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () y)))
	       (funcall (closure a) 5 2 7))) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () z)))
	       (funcall (closure a) 5 2 7))) 		'eq? 7)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () (+ x x))))
	      (funcall (closure a) 5 2 7))) 		'eq? 10)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () (+ x z))))
	      (funcall (closure a) 5 2 7))) 		'eq? 12)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) () (+ x (- z 3)))))
	      (funcall (closure a) 5 2 7))) 		'eq? 9)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) ()
			   (let ((k (+ x 1)))
			     (+ k (- z 3))))))
	      (funcall (closure a) 5 2 7))) 		'eq? 10)
(run-test '(labels ((a (code () () 123)) (b (code (x) () (- 3 x))))
	     (funcall (closure b) (funcall (closure a))))			'eq? -120)
