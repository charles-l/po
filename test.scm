...
(run-test '(labels ((a (code (x y z) x)))
	     (labelcall a 4 1 2)) 	'eq? 4)
(run-test '(labels ((a (code (x y z) y)))
	     (labelcall a 4 1 2)) 		'eq? 1)
(run-test '(labels ((a (code (x y z) z)))
	     (labelcall a 4 1 2)) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) x)))
	       (labelcall a 5 2 7))) 		'eq? 5)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) y)))
	       (labelcall a 5 2 7))) 		'eq? 2)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) z)))
	       (labelcall a 5 2 7))) 		'eq? 7)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) (+ x x))))
	      (labelcall a 5 2 7))) 		'eq? 10)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) (+ x z))))
	      (labelcall a 5 2 7))) 		'eq? 12)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z) (+ x (- z 3)))))
	      (labelcall a 5 2 7))) 		'eq? 9)
(run-test '(let ((n 0))
	     (labels ((a (code (x y z)
			   (let ((k (+ x 1)))
			     (+ k (- z 3))))))
...
(run-test '(labels ((a (code () 123)) (b (code (x) (- 3 x))))
	     (labelcall b (labelcall a)))			'eq? -120)
